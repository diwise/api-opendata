...
func NewRetrieveWaterQualityHandler(ctx context.Context, svc waterquality.WaterQualityService) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if maxDistance != "" {
			...
			if err != nil {
				log.Error("failed to parse distance from query parameters", "error", err)
		...
		if acceptedContentType == geoJSONContentType {
			...
			if err != nil {
				log.Error("failed to marshal beach list to GeoJson", "error", err)
			...
			if err != nil {
				log.Error("failed to marshal water quality into json", "error", err)
				w.WriteHeader(http.StatusInternalServerError)
				return
			}
...
func NewRetrieveWaterQualityByIDHandler(ctx context.Context, svc waterquality.WaterQualityService) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Error("failed to parse parameters from query", "error", err)
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		...
		if len(values) != 0 {
			if values.Get("from") != "" {
				...
				if err != nil {
					log.Error("time parameter from is incorrect format", "error", err)
					w.WriteHeader(http.StatusBadRequest)
					return
				}
			}
			if values.Get("to") != "" {
				...
				if err != nil {
					log.Error("time parameter to is incorrect format", "error", err)
					w.WriteHeader(http.StatusBadRequest)
					return
				}
			}
		...
		if err != nil {
			log.Error("no water quality found", "error", err, "id", waterqualityID)
		...
		if err != nil {
			log.Error("failed to marshal water quality", "error", err)
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
